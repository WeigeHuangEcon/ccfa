...
```{r}
library(quantreg)
out <- cfa2(lcfincome ~ lfincome, t, y_0, data, method1="qr", xformla2=~HEDUC, method2="qr", iters=25, tau1=seq(.02,.98,.02), tau2=seq(.02,.98,.02))
```
...
```{r}
ggplot2.CFA(test.CFA(out$cfa1, Var, data$lfincome), setype="uniform")
```
...
```{r}
llres <- cfa(lcfincome ~ lfincome, ~HEDUC, tvals=t, yvals=y_0, method="ll", se=FALSE, data=data)
ggplot2.CFA(getRes.CFA(temp, E, se=FALSE))
```
